...
public class SimpleTelegramBot extends TelegramLongPollingBot {
    ...
    private void handlePhotoMessage(Message message) {
        try {
            ...
            if (photo.getFileSize() != null && photo.getFileSize() > 1024 * 1024) { // 1MB вместо 2MB
                sendMessage(message.getChatId(), 
                    "⚠️ Изображение слишком большое (" + (photo.getFileSize() / 1024 / 1024) + " МБ).\n\n" +
                    "📸 Для экономии токенов нужны компактные файлы до 1 МБ:\n" +
                    "• Сжать фото в галерее до 1 МБ\n" +
                    "• Использовать меньшее разрешение камеры\n" +
                    "• Убрать лишние детали из кадра\n\n" +
                    "💰 Это позволит использовать в 3-4 раза меньше токенов");
                return;
            }
